...
impl TcpCall {
    pub fn new(key_store: Arc<Keystore>, device_cache: Arc<DeviceCache>, owner: WeakSession, timeout: Duration, remote: Endpoint) -> Self {
    ...
    async fn call_proc(&self) -> BdtResult<BdtResult<Device>> {
        ...
        let interface = Interface::connect(
            ...
            self.0.device_cache.get_inner(packages.remote()).unwrap().desc().clone(),
            packages.key().clone(),
        ...
        let resp_box = interface.confirm_connect(self.0.key_store.as_ref(), vec![DynamicPackage::from(sn_call.clone())], self.0.timeout - escaped).await?;
...
impl CallTunnel for TcpCall {
    ...
    fn reset(&self, timeout: Duration) -> Option<Box<dyn CallTunnel>> {
        Some(Box::new(Self(Arc::new(TcpImpl {
            ...
            device_cache: self.0.device_cache.clone(),
        }))))
    }
