...
impl CallManager {
    ...
    pub async fn call(
        ...
        for sn_id in sn_list {
            let sn = self.0.device_cache.get_inner(sn_id).ok_or_else(|| BuckyError::new(BuckyErrorCode::NotFound, "sn device not cached"))?;
            let mut call = SnCall {
                protocol_version: 0,
                stack_version: 0,
                seq: seq,
            ...
            if let Some(active) = self.0.sn_cache.get_active(sn_id) {
                ...
                if sn.connect_info().endpoints().iter().find(|ep| active.remote().eq(ep)).is_some() {
                    if active.is_udp() {
                        ...
                        if net_listener.tcp_of(active.local()).is_some() {
                            let tunnel = TcpCall::new(self.0.key_store.clone(),
                                                      self.0.device_cache.clone(),
                                                      session.to_weak(),
                                                      self.0.call_config.timeout,
                                                      active.remote().clone());
                            session.add_tunnel(tunnel.clone_as_call_tunnel());
                            cached = true;
                        }
            ...
            if !cached {
                ...
                self.0.sn_cache.remove_active(sn_id);
                {
...
impl CallSession {
    ...
    async fn new(key_store: Arc<Keystore>, device_cache: Arc<DeviceCache>, sn_cache: Arc<SnCache>, local_device_id: DeviceId, call: SnCall, config: CallConfig) -> Self {
        ...
        if let keystore::EncryptedKey::Unconfirmed(encrypted) = &key_stub.encrypted {
            ...
            let _ = exchange.sign(key_store.signer()).await.unwrap();
    ...
    fn sync_tunnel(&self, _tunnel: &dyn CallTunnel, result: BdtResult<Device>, active: Option<EndpointPair>) {
        ...
        if let Some(endpoint) = next.update_cache {
            self.0.sn_cache.add_active(self.sn(), endpoint);
        }
    ...
    async fn reset(&self, call: SnCall) {
        self.0.key_store.reset_peer(self.sn());

        let key_stub = self.0.key_store.create_key(self.0.device_cache.get_inner(self.sn()).unwrap().desc(), true);
        let mut packages = PackageBox::encrypt_box(self.sn().clone(), key_stub.key.clone());
        if let keystore::EncryptedKey::Unconfirmed(encrypted) = &key_stub.encrypted {
            let mut exchange = Exchange::from((&call, encrypted.clone(), key_stub.key.mix_key.clone()));
            let _ = exchange.sign(self.0.key_store.signer()).await.unwrap();
            packages.push(exchange);
        }
        packages.push(call);
