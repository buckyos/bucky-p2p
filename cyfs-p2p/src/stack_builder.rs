...
use bucky_objects::{
    verify_object_desc_sign, Device, Endpoint, NamedObject, ObjSignature, ObjectDesc, Protocol,
    RsaCPUObjectVerifier, SignatureSource, SingleKeyObjectDesc, SIGNATURE_SOURCE_REFINDEX_SELF,
};
...
impl P2pIdentityCert for CyfsIdentityCert {
    ...
    fn verify_cert(&self, name: &str) -> bool {
        ...
        block_on(verify_object_desc_sign(
            &verifier,
            &self.device,
            signs.unwrap(),
        ))
    ...
    fn get_encoded_cert(&self) -> P2pResult<EncodedP2pIdentityCert> {
        self.device.to_vec().map_err(into_p2p_err!(
            P2pErrorCode::CertError,
            "encode device to vec failed"
        ))
    }
    ...
    fn endpoints(&self) -> Vec<p2p_frame::endpoint::Endpoint> {
        self.device
            ...
            .endpoints()
            .iter()
            .filter(|ep| !ep.addr().ip().is_unspecified())
            .map(|ep| p2p_frame::endpoint::Endpoint::from_str(ep.to_string().as_str()).unwrap())
            .collect::<Vec<_>>()
    }
...
impl P2pIdentity for CyfsIdentity {
    ...
    fn sign(&self, message: &[u8]) -> P2pResult<P2pSignature> {
        self.key
            .sign(message)
            .map_err(into_p2p_err!(P2pErrorCode::SignError, "sign error"))?
            .to_vec()
            .map_err(into_p2p_err!(P2pErrorCode::SignError, "sign error"))
    }

    fn get_encoded_identity(&self) -> P2pResult<EncodedP2pIdentity> {
        self.to_vec().map_err(into_p2p_err!(
            P2pErrorCode::Failed,
            "encode identity to vec failed"
        ))
    }
...
impl P2pIdentityFactory for CyfsIdentityFactory {
    fn create(&self, id: &EncodedP2pIdentity) -> P2pResult<P2pIdentityRef> {
        Ok(Arc::new(
            CyfsIdentity::clone_from_slice(id.as_slice()).map_err(into_p2p_err!(
                P2pErrorCode::Failed,
                "decode identity from vec failed"
            ))?,
        ))
    }
}
...
impl P2pIdentityCertFactory for CyfsIdentityCertFactory {
    fn create(&self, cert: &EncodedP2pIdentityCert) -> P2pResult<P2pIdentityCertRef> {
        let device = Device::clone_from_slice(cert.as_slice()).map_err(into_p2p_err!(
            P2pErrorCode::Failed,
            "decode device from vec failed"
        ))?;
...
pub fn cyfs_to_p2p_endpoint(ep: &Endpoint) -> p2p_frame::endpoint::Endpoint {
    match ep.protocol() {
        Protocol::Unk => p2p_frame::endpoint::Endpoint::from((
            p2p_frame::endpoint::Protocol::Quic,
            ep.addr().clone(),
        )),
        Protocol::Tcp => p2p_frame::endpoint::Endpoint::from((
            p2p_frame::endpoint::Protocol::Tcp,
            ep.addr().clone(),
        )),
        Protocol::Udp => p2p_frame::endpoint::Endpoint::from((
            p2p_frame::endpoint::Protocol::Quic,
            ep.addr().clone(),
        )),
    }
...
pub fn create_cyfs_p2p_config(
    ep: Vec<p2p_frame::endpoint::Endpoint>,
    server_runtime: ServerRuntime,
) -> P2pConfig {
    let identity_factory = Arc::new(CyfsIdentityFactory);
    let cert_factory = Arc::new(CyfsIdentityCertFactory);
    P2pConfig::new(identity_factory, cert_factory, ep, server_runtime)
}

pub fn create_cyfs_p2p_stack_config(
    env: P2pEnvRef,
    ...
    for sn in sn_list {
        ...
        p2p_sn_list.push(P2pSn::new(
            p2p_id.clone(),
            p2p_id.to_string(),
            sn.connect_info()
                .endpoints()
                .iter()
                .map(|ep| cyfs_to_p2p_endpoint(ep))
                .collect(),
        ));
    }
    P2pStackConfig::new(env, Arc::new(CyfsIdentity::new(local_identity, local_key)))
        .add_sn_list(p2p_sn_list)
}
