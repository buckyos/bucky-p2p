...
impl QuicSocket {
    ...
    pub async fn connect(local_device_ref: LocalDeviceRef,
        ...
        let client_key = local_device_ref.get_encoded_identity()?;
        let client_cert = local_device_ref.get_identity_cert()?.get_encoded_cert()?;
        let mut config =
