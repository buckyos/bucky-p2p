    ...
    pub async fn connect(
        ...
        let client_config =
            rustls::ClientConfig::builder_with_provider(crate::tls::provider().into())
                ...
                .with_client_auth_cert(vec![CertificateDer::from(local_device_ref.get_name().to_vec().unwrap())],
                                       PrivatePkcs8KeyDer::from(local_device_ref.get_encoded_identity()?).into()).unwrap();
        ...
        let tls_stream = tls_connector.connect(remote_device_id.to_string().try_into().unwrap(), socket).await.map_err(into_bdt_err!(BdtErrorCode::ConnectFailed, "tls socket to {} connect failed", remote_ep))?;
        let socket = TCPSocket::new(runtime::TlsStream::from(tls_stream), local_device_ref.get_id(), remote_device_id, local, remote_ep);
