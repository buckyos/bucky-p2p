...
use crate::error::{into_bdt_err, BdtError, BdtErrorCode};
...
impl SqliteStorage {
    ...
    pub async fn init(&mut self, service_name: &str) -> Result<(), Box<dyn Error>> {
        ...
        let dir = std::env::current_dir().unwrap().join("../../../../../profile").join(service_name);
        if !dir.is_dir() {
...
impl super::AsyncStorage for SqliteStorage {
    async fn set_item(&mut self, key: &str, value: String) -> Result<(), BdtError> {
        ...
        let conn = Connection::open(self.path.as_path()).map_err(into_bdt_err!(BdtErrorCode::Failed, "open db failed, db={}", self.path.display()))?;
        let sql = format!("REPLACE INTO {} (key, value) VALUES (?1, ?2)", TABLE_NAME);
        conn.execute(&sql, rusqlite::params![key, value])
            .map_err(into_bdt_err!(BdtErrorCode::Failed, "[sqlite] set item failed"))?;
        Ok(())
    }
