...
use crate::protocol::v0::{
    AckClose, AckDatagram, AckReverseDatagram, AckReverseSession, AckSession, SynClose,
    SynDatagram, SynReverseDatagram, SynReverseSession, SynSession, TunnelType,
};
use crate::protocol::{MTU_LARGE, Package, PackageCmdCode, PackageHeader};
use crate::runtime;
...
impl Display for TunnelSession {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            TunnelSession::Forward((session, _, _)) => write!(
                f,
                "Forward(seq {:?} session {} port {})",
                session.tunnel_id, session.session_id, session.to_vport
            ),
            TunnelSession::Reverse((session, _, _)) => write!(
                f,
                "Reverse(seq {:?} session {} port {})",
                session.tunnel_id, session.session_id, session.vport
            ),
        }
    }
}
...
async fn accept_pkg(mut read: P2pReadHalf) -> P2pResult<(P2pReadHalf, PackageCmdCode, Vec<u8>)> {
    ...
    loop {
        read.read_exact(&mut buf_header[0..PackageHeader::raw_bytes().unwrap()])
            .await
        ...
        read.read_exact(cmd_body.as_mut_slice())
            .await
            .map_err(into_p2p_err!(P2pErrorCode::IoError))?;
        if cmd_code == PackageCmdCode::SynSession
            || cmd_code == PackageCmdCode::AckSession
            || cmd_code == PackageCmdCode::SynReverseSession
            || cmd_code == PackageCmdCode::AckReverseSession
...
pub type TunnelFutureHolderRef =
    Arc<NotifyHolder<P2pResult<(P2pReadHalf, PackageCmdCode, Vec<u8>)>>>;
...
impl TunnelConnectionState {
    pub(crate) fn new(tunnel_id: TunnelId, data_socket: P2pConnection) -> Self {
...
impl TunnelConnectionRead {
    ...
    async fn read_exact_from_holder(
        read: ReadHolder,
        tunnel_id: TunnelId,
        local_id: P2pId,
        buf: &mut [u8],
        ...
        if let Some(read) = read_guard.as_mut() {
            let ret = read
                .read_exact(buf)
                .await
                .map_err(into_p2p_err!(P2pErrorCode::IoError))?;
    ...
    async fn read_next_cmd(&mut self) -> P2pResult<PackageCmdCode> {
        let mut buf_header = [0u8; 16];
        let tunnel_id = self.conn.get_tunnel_id();
        let local_id = self.local_id.clone();
        let read = self.read.clone();
        Self::read_exact_from_holder(
            read.clone(),
            tunnel_id,
            local_id.clone(),
            &mut buf_header[0..PackageHeader::raw_bytes().unwrap()],
        )
        .await?;
        let header = PackageHeader::clone_from_slice(buf_header.as_slice())
            .map_err(into_p2p_err!(P2pErrorCode::RawCodecError))?;
    ...
    async fn recv_inner(&mut self, buf: &mut [u8]) -> P2pResult<usize> {
        ...
        if self.remainder == 0 {
            let cmd_code = self.read_next_cmd().await?;
        ...
        if self.remainder as usize > buf.len() {
            ...
            let recv_len = self.remainder;
            {
                let ret = self
                    ...
                    .as_mut()
                    .unwrap()
                    .read_exact(&mut buf[..self.remainder as usize])
                    .await
                    .map_err(into_p2p_err!(P2pErrorCode::IoError))?;
    ...
    fn close(&mut self) {
        ...
        let local_id = self.local_id.clone();
        let initial_remainder = self.remainder;
        Executor::spawn_ok(async move {
            if read_state == ReadState::Closed {
                ...
                let read = {
                    ...
                    if state.tunnel_state == TunnelState::Worked {
                        log::info!(
                            "tunnel {:?} state from {:?} to ReadClosed local {}",
                            tunnel_id,
                            state.tunnel_state,
                            local_id.to_string()
                        );
            ...
            } else if read_state == ReadState::Work {
                let mut read = read.get().await.take().unwrap();
                let out_conn = conn.clone();
                let (read, ret): (P2pReadHalf, P2pResult<()>) = async move {
                    if initial_remainder > 0 {
                        let mut buf = vec![0u8; initial_remainder as usize];
                        let ret = read
                    ...
                    loop {
                        ...
                        let ret = read
                            .read_exact(buf_header.as_mut_slice())
                            .await
                            .map_err(into_p2p_err!(P2pErrorCode::IoError));
                        let ret = match ret {
                            Ok(ret) => ret,
                ...
                if ret.is_ok() {
                    let read = {
                        ...
                        if state.tunnel_state == TunnelState::Worked {
                            log::info!(
                                "tunnel {:?} state from {:?} to ReadClosed",
                                tunnel_id,
                                state.tunnel_state
                            );
                            state.tunnel_state = TunnelState::ReadClosed;
                    ...
                    log::info!(
                        "tunnel {:?} state from {:?} to Error.err {:?}",
                        tunnel_id,
                        state.tunnel_state,
                        ret.err()
                    );
    ...
    fn poll_read(
        ...
        unsafe {
            if self.buf_cache_len > 0 && self.buf_cache_len > self.buf_read_pos {
                let read_len =
                    std::cmp::min(self.buf_cache_len - self.buf_read_pos, buf.remaining());
    ...
    async fn send_inner(&mut self, data: &[u8]) -> std::io::Result<()> {
        ...
        while remainder.len() > 0 {
            ...
            self.write
                .as_mut()
                .unwrap()
                .write_all(header.to_vec().unwrap().as_slice())
                .await?;
    ...
    fn close(&mut self) {
        ...
        Executor::spawn_ok(async move {
            ...
            let ret: P2pResult<()> = async move {
                ...
                write
                    .write_all(pkg.to_vec().unwrap().as_slice())
                    .await
                    .map_err(into_p2p_err!(P2pErrorCode::IoError))?;
                write
                    .flush()
                ...
                let read = {
                    ...
                    if state.tunnel_state == TunnelState::Worked {
                        log::info!(
                            "tunnel {:?} state from {:?} to WriteClosed",
                            tunnel_id,
                            state.tunnel_state
                        );
                        state.tunnel_state = TunnelState::WriteClosed;
            ...
            if let Err(err) = ret {
                ...
                state.tunnel_state = TunnelState::Error;
                log::info!(
                    "close stream {:?} local_id {} remote_id {} err {}",
                    tunnel_id,
                    local_id.to_string(),
                    tunnel.remote_id.to_string(),
                    err
                );
            }
        });
...
impl TunnelConnection {
    pub fn new(
        ...
        log::info!(
            "create tunnel connection {:?} local_id {} remote_id {} remote_ep {}",
            tunnel_id,
            local_identity.get_id().to_string(),
            remote_id.to_string(),
            remote_ep
        );
        ...
        Arc::new(Self {
            ...
            conn_timeout,
            protocol_version,
            state: Mutex::new(TunnelConnectionState::new(tunnel_id, tcp_socket)),
    ...
    fn set_tunnel_error(&self) {
        self.set_tunnel_state(TunnelState::Error);
    }

    fn take_write_for_open(&self) -> P2pResult<(TunnelId, P2pWriteHalf)> {
        ...
        let write = inner.write.take().ok_or_else(|| {
            p2p_err!(
                P2pErrorCode::ErrorState,
                "tunnel {:?} missing write half",
                inner.tunnel_id
            )
        })?;
        Ok((inner.tunnel_id, write))
    }
    ...
    fn enter_idle_mode(&self, read: P2pReadHalf) -> P2pResult<()> {
        ...
        let handle =
            create_accept_handle(read, self.accept_future.clone(), self.recv_future.clone())?;
        state.accept_handle = Some(handle);
        state.tunnel_state = TunnelState::Idle;
        log::info!(
            "enter idle mode tunnel {:?} local_id {} remote_id {}",
            state.tunnel_id,
    ...
    fn wait_pkg(
        &self,
        holder: TunnelFutureHolderRef,
    ) -> NotifyWaiter<P2pResult<(P2pReadHalf, PackageCmdCode, Vec<u8>)>> {
        let (notify, waiter) = Notify::new();
        holder.set_future(notify);
        let read = {
            let mut state = self.state.lock().unwrap();
            state.read.take()
        };
        if let Some(read) = read {
            if let Err(err) = self.enter_idle_mode(read) {
                holder.try_complete(Err(err));
            }
        }
        waiter
    }
    ...
    fn accept(&self) -> NotifyWaiter<P2pResult<(P2pReadHalf, PackageCmdCode, Vec<u8>)>> {
        self.wait_pkg(self.accept_future.clone())
    }

    fn recv(&self) -> NotifyWaiter<P2pResult<(P2pReadHalf, PackageCmdCode, Vec<u8>)>> {
        self.wait_pkg(self.recv_future.clone())
    }

    async fn open_session_impl<S, A, FBuild, FDecode>(
        self: &Arc<Self>,
        session_type: TunnelType,
        vport: u16,
        session_id: SessionId,
        syn_cmd: PackageCmdCode,
        ack_cmd: PackageCmdCode,
        build_syn: FBuild,
        decode_ack: FDecode,
    ) -> P2pResult<(A, TunnelConnectionRead, TunnelConnectionWrite)>
    where
        S: RawEncode,
        FBuild: FnOnce(TunnelId, TunnelType, u16, SessionId) -> S,
        FDecode: FnOnce(&[u8]) -> P2pResult<A>,
    {
        let (tunnel_id, mut write) = self.take_write_for_open()?;
        let recv_future = self.recv();
        let syn = build_syn(tunnel_id, session_type, vport, session_id.clone());
        let pkg = Package::new(self.protocol_version, syn_cmd, syn);

        let result = runtime::timeout(self.conn_timeout, async {
            write
                .write_all(pkg.to_vec().unwrap().as_slice())
                .await
                .map_err(into_p2p_err!(P2pErrorCode::IoError))?;
            ...
            if cmd_code != ack_cmd {
                return Err(p2p_err!(
                    ...
                    "tunnel {:?} invalid ack stream",
                    tunnel_id
                ));
            }
            let ack = decode_ack(cmd_body.as_slice())?;
            Ok((ack, read))
        })
        .await;
        ...
        match result {
            Ok(Ok((ack, read))) => {
                self.set_tunnel_state(TunnelState::Worked);
                Ok((
                    ack,
                    TunnelConnectionRead::new(self.clone(), read, session_id.clone(), vport),
                    TunnelConnectionWrite::new(self.clone(), write, session_id, vport),
                ))
            }
            Ok(Err(err)) => {
                self.set_tunnel_error();
                Err(err)
            }
            Err(err) => {
                self.set_tunnel_error();
    ...
    fn enter_accepting_mode(&self, tunnel_id: TunnelId) -> Option<P2pWriteHalf> {
        let mut inner = self.state.lock().unwrap();
        ...
        log::info!(
            "tunnel {:?} state from {:?} to Accepting",
            tunnel_id,
            inner.tunnel_state
        );
        inner.tunnel_state = TunnelState::Accepting;
        inner.tunnel_id = tunnel_id;
        inner.write.take()
    }
    ...
    pub(crate) async fn open_reverse_session(
        self: &Arc<Self>,
        session_type: TunnelType,
        vport: u16,
        session_id: SessionId,
        ...
        self.open_session_impl::<SynReverseSession, AckReverseSession, _, _>(
            session_type,
            vport,
            session_id,
            PackageCmdCode::SynReverseSession,
            PackageCmdCode::AckReverseSession,
            move |tunnel_id, tunnel_type, vport, session_id| SynReverseSession {
                tunnel_type,
                tunnel_id,
                session_id,
                vport,
            ...
            |cmd_body| {
                AckReverseSession::clone_from_slice(cmd_body)
                    .map_err(into_p2p_err!(P2pErrorCode::RawCodecError))
            },
        )
        .await
    ...
    pub(crate) async fn open_session(
        ...
        vport: u16,
        session_id: SessionId,
    ) -> P2pResult<(AckSession, TunnelConnectionRead, TunnelConnectionWrite)> {
        self.open_session_impl::<SynSession, AckSession, _, _>(
            session_type,
            vport,
            session_id,
            PackageCmdCode::SynSession,
            PackageCmdCode::AckSession,
            |tunnel_id, tunnel_type, vport, session_id| SynSession {
                tunnel_type,
                tunnel_id,
                to_vport: vport,
                session_id,
            ...
            |cmd_body| {
                AckSession::clone_from_slice(cmd_body)
                    .map_err(into_p2p_err!(P2pErrorCode::RawCodecError))
            },
        )
        .await
    ...
    pub(crate) async fn accept_session(self: &Arc<Self>) -> P2pResult<TunnelSession> {
        loop {
            ...
            match cmd_code {
                PackageCmdCode::SynSession => {
                    let syn_stream = SynSession::clone_from_slice(cmd_body.as_slice())
                        .map_err(into_p2p_err!(P2pErrorCode::RawCodecError))?;
                    let vport = syn_stream.to_vport;
                    let session_id = syn_stream.session_id;
                    let write = match self.enter_accepting_mode(syn_stream.tunnel_id) {
                        Some(write) => write,
                        None => continue,
                    };
                    return Ok(TunnelSession::Forward((
                        syn_stream,
                        TunnelConnectionRead::new(self.clone(), read, session_id, vport),
                        TunnelConnectionWrite::new(self.clone(), write, session_id, vport),
                    )));
                }
                PackageCmdCode::SynReverseSession => {
                    let reserve_stream = SynReverseSession::clone_from_slice(cmd_body.as_slice())
                        .map_err(into_p2p_err!(P2pErrorCode::RawCodecError))?;
                    let vport = reserve_stream.vport;
                    let session_id = reserve_stream.session_id;
                    let write = match self.enter_accepting_mode(reserve_stream.tunnel_id) {
                        Some(write) => write,
                        None => continue,
                    };
                    return Ok(TunnelSession::Reverse((
                        reserve_stream,
                        TunnelConnectionRead::new(self.clone(), read, session_id, vport),
                        TunnelConnectionWrite::new(self.clone(), write, session_id, vport),
                    )));
    ...
    fn shutdown(&self) -> P2pResult<()> {
        ...
        self.recv_future.try_complete(Err(p2p_err!(
            P2pErrorCode::Interrupted,
            "tunnel {:?} shutdown",
            tunnel_id
        )));
        self.accept_future.try_complete(Err(p2p_err!(
            P2pErrorCode::Interrupted,
            "tunnel {:?} shutdown",
            tunnel_id
        )));
...
impl Drop for TunnelConnection {
    fn drop(&mut self) {
        {
            log::info!(
                "drop tunnel connection {:?}",
                self.state.lock().unwrap().tunnel_id
            );
        }
        self.shutdown();
    }
