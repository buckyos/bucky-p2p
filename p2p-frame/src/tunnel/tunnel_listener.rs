...
use crate::error::{p2p_err, P2pResult};
use crate::executor::*;
...
use crate::tunnel::{TunnelConnection, TunnelConnectionRead, TunnelConnectionRef, TunnelConnectionWrite, TunnelSession};
...
impl TunnelListener {
    pub fn new(local_identity: P2pIdentityRef,
               ...
               protocol_version: u8,
               cert_factory: P2pIdentityCertFactoryRef,
               conn_timeout: Duration,) -> TunnelListenerRef {
    ...
    pub fn listen(self: &Arc<Self>) {
        let this = self.clone();
        self.net_manager.set_connection_event_listener(self.local_identity.get_id(), move |conn| {
            let this = this.clone();
            async move {
    ...
    async fn accept_tunnel(&self, conn: P2pConnection) -> P2pResult<()> {
        ...
        match tunnel_conn.accept_session().await? {
            TunnelSession::Forward((session, read, write)) => {
                let listener = {
                    let state = self.state.lock().unwrap();
                    state.waiter_map.get(&session.tunnel_type).map(|v| v.clone())
                };
                if let Some(listener) = listener {
                    listener.on_new_tunnel(session, tunnel_conn.clone(), read, write).await?;
                }
                Ok(())
            },
            TunnelSession::Reverse((session, read, write)) => {
                ...
                if let Some(listener) = listener {
                    listener.on_new_reverse_tunnel(session, tunnel_conn.clone(), read, write).await?;
                }
                Ok(())
            }
        }
