...
- `p2p-frame/src/sn/service/service.rs` 收紧 runtime 所有权后，`SnServiceConfig::new(...)` 的显式 runtime 注入路径和服务内部不得自行 `ServerRuntime::start(...)` 会成为新的配置回归热点。
- `p2p-frame/src/stack.rs` 收紧 stack 级 runtime 所有权后，`P2pConfig::new(...)` 必须显式接收 `ServerRuntime`，`create_p2p_env(...)` 不得在缺省时自行 `ServerRuntime::start(...)`，下游构造调用点必须通过编译暴露未迁移路径。
...
| SN service runtime 外部注入 | `p2p-frame/src/sn/service/service.rs` 的 `SnServiceConfig::new(...)`、`SnServiceConfig::set_server_runtime(...)`、`sn-miner-rust` / `cyfs-p2p-test` 启动调用点 | targeted unit: `cargo test -p p2p-frame create_sn_service_accepts_external_server_runtime -- --nocapture`; workspace compile: `cargo test --workspace --no-run`; unified unit/integration: `python3 ./harness/scripts/test-run.py p2p-frame unit` / `python3 ./harness/scripts/test-run.py p2p-frame integration`; search: `rg -n 'ServerRuntime::start|ServerRuntimeConfig' p2p-frame/src/sn/service/service.rs` | unit 证据确认显式 runtime 注入构造路径可用；workspace 编译和 integration 证据确认 `sn-miner-rust` / `cyfs-p2p-test` 调用方迁移为显式 runtime 注入且构造路径仍可用；搜索证据确认 SN service 生产路径不再创建 service-local runtime。 |
| stack 级 `ServerRuntime` 外部传入 | `p2p-frame/src/stack.rs` 的 `P2pConfig::new(...)`、`P2pConfig::server_runtime(...)` 和 `create_p2p_env(...)`，以及 `cyfs-p2p` / `cyfs-p2p-test` 构造调用点 | compile: `cargo check -p p2p-frame --lib` / `cargo check -p cyfs-p2p --lib` / `cargo check -p cyfs-p2p-test`; targeted tests compile/run: `cargo test -p p2p-frame create_sn_service_accepts_external_server_runtime -- --nocapture`; search: `rg -n 'ServerRuntime::start|ServerRuntimeConfig' p2p-frame/src/stack.rs cyfs-p2p/src/stack_builder.rs cyfs-p2p-test/src/main.rs` | 编译证据确认 `P2pConfig::new(...)` 调用方必须显式传入 `ServerRuntime`；搜索证据确认 `stack.rs` 不再调用 `ServerRuntime::start(...)` 且外部 `cyfs-p2p` helper 负责创建 runtime；测试编译覆盖 SN helper 内直接 client stack 构造已迁移为外部 runtime。 |
...
| stack_server_runtime_external_required | normal | yes | V-STACK-RUNTIME-EXTERNAL-COMPILE | unit | covered | |
| stack_server_runtime_external_required | boundary | yes | V-STACK-RUNTIME-EXTERNAL-COMPILE | unit | covered | |
| stack_server_runtime_external_required | negative | yes | V-STACK-RUNTIME-EXTERNAL-COMPILE | unit | covered | |
| stack_server_runtime_external_required | error | yes | V-STACK-RUNTIME-EXTERNAL-COMPILE | unit | covered | |
| stack_server_runtime_external_required | compatibility | yes | V-STACK-RUNTIME-EXTERNAL-COMPAT | integration | covered | |
| stack_server_runtime_external_required | lifecycle | yes | V-STACK-RUNTIME-EXTERNAL-COMPILE | unit | covered | |
| stack_server_runtime_external_required | cross-module | yes | V-STACK-RUNTIME-EXTERNAL-COMPAT | integration | covered | |
...
| invariant | approved proposal `sn_service_runtime_external_injection`; `design.md` SN service runtime external injection | missing runtime is rejected by constructor signature at compile time | unit | covered | |
...
| parameter-domain | approved proposal `stack_server_runtime_external_required`; `design.md` Stack Runtime Ownership; `design/sfo-reuseport-listeners.md` Runtime Ownership | externally supplied `ServerRuntime`, migrated direct callers | unit | covered | |
| invariant | approved proposal `stack_server_runtime_external_required`; `design.md` Key Call Flows | `create_p2p_env(...)` does not start a default `ServerRuntime` | unit | covered | |
| invariant | approved proposal `stack_server_runtime_external_required`; `design.md` Interfaces and Dependencies | `cyfs-p2p` and `cyfs-p2p-test` compile after direct caller migration | integration | covered | |
...
| SN service 不再内部创建 runtime | targeted SN service unit `create_sn_service_accepts_external_server_runtime`, workspace compile / integration, and source search | `SnServiceConfig::new(...)` 必须接收 runtime，`sn-miner-rust` / `cyfs-p2p-test` 显式创建并传入进程级 runtime，源码搜索确认 `service.rs` 无 `ServerRuntime::start` fallback | |
| stack 不再内部创建 runtime | p2p-frame/cyfs-p2p/cyfs-p2p-test compile and source search | `P2pConfig::new(...)` 的签名把 `ServerRuntime` 变为必填参数，编译覆盖直接调用点迁移；源码搜索确认 `stack.rs` 无 `ServerRuntime::start(...)` fallback，runtime 创建发生在外部适配层 | |
...
| `SnServiceConfig::new(...)` | explicit server runtime argument | 必须传入 `ServerRuntime`，不启动 service-local `ServerRuntime` | `p2p-frame/src/sn/service/service.rs` | covered | |
| `P2pConfig::new(...)` / `create_p2p_env(...)` | stack-level server runtime ownership | 构造时必须显式传入 `ServerRuntime`，env 创建只克隆已传入 runtime，不在 `stack.rs` 内启动默认 runtime | `p2p-frame/src/stack.rs` | covered | |
