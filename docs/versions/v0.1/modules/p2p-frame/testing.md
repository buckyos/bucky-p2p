...
approved_at: 2026-06-18T16:35:00+08:00
approved_content_sha256: d395425545e660660af09a8a196678bd5ef9be625d284bfaf3d9886e7ac96114
---
...
| `ttp` | 复用命令/流协议 | `p2p-frame/docs/ttp_module_design.md` | 流注册、server/client/node 协议交互，`TtpNode` 缺失 tunnel 时主动创建并 attach 后打开 stream/control stream/datagram，已有 tunnel 时按 target 复用 | 无效命令流、channel 关闭、`TtpServer` lookup-only 行为保持、`TtpNode` 建链失败显式返回错误 | unit | `p2p-frame/src/ttp/tests.rs` |
