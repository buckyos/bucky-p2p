...
approved_content_sha256: 7621740f315dbf2b169cf2ea40dc1bd53cb477ba92517128266d2387c3125971
---
...
- 为本轮 SN server TCP 来源地址收窄建立可执行设计边界：SN server 不能把 TCP command tunnel 的来源 socket address 作为普通候选返回或写入 peer endpoint 状态；只有客户端上报 `map_ports` 时，服务端可以临时使用 TCP 来源 IP 与上报端口构造 `Mapped` endpoint。
...
- 为本轮 `TtpServer` incoming tunnel validator 需求建立可执行设计边界：新增 server 本地 validator 装配点和默认 allow-all helper，`TtpServer` 在收到 `NetManager` incoming tunnel 后先基于 tunnel 元数据执行 validator，accept 后才 attach/remember，reject 时不 attach、不记入本地 cache，并尽力关闭被拒 tunnel。
...
- 为本轮 SN client 协议优先级建立可执行设计边界：同一 SN 服务端同时存在 QUIC/TCP endpoint 且本地支持对应出站连接协议时，客户端 ping/report 建链按 QUIC 优先、TCP fallback 顺序尝试；是否尝试某协议不要求本地已监听该协议端口；如果本地存在匹配 QUIC listener，建链分类必须继续使用该 listener `local_ep` 保持同源 UDP 语义；如果本地存在匹配 TCP listener，TCP fallback 不得把该已监听端口原样作为 outbound socket bind 地址，必须改用 ephemeral local port 或无本地端点；某个 SN 的 QUIC command tunnel 建立且 `ReportSn` 成功后停止该 SN 的 TCP 尝试；只有 QUIC 建链或 report 失败后才进入 TCP fallback，并避免同一成功 SN 重复进入 active SN 列表。
...
- 把 TCP command tunnel 的来源 socket address 当作 SN 可返回或可缓存的普通候选 endpoint；用原始 TCP 来源端口构造候选；改变 SN command wire、TCP tunnel wire、TLS 身份校验或 QUIC `ServerReflexive` 语义
...
- 借 `TtpServer` incoming tunnel validator 改变 `NetManager` / `TunnelNetwork` publish 规则、底层传输认证、TCP/QUIC/PN/TTP wire、`TtpNode` active-open 行为或 `TtpClient` 生命周期语义
...
- 将 SN client 协议优先级解释为改变 SN command wire、control-stream-only 信令、TCP/QUIC tunnel wire、TLS 身份校验、TTP target matching、`TtpClient::connect_server(...)` maintained target 语义、入站 listener/上报候选语义、服务端选择 API 或跨 SN fanout；TCP 只作为同一 SN 上 QUIC 建链/report 失败后的 fallback，不用于降低远端 QUIC/UDP 候选优先级；未监听本地端口不得被解释为不支持出站协议，已有匹配 QUIC listener 时不得丢弃其 `local_ep`；已有匹配 TCP listener 时不得复用监听端口作为 outbound bind 地址。
...
- Existing components or patterns reused: 复用现有 `TtpClient` / `TtpServer`、TTP target 匹配、`TtpRuntime::attach_tunnel(...)`、`IncomingTunnelValidateContext` 风格的 validator、`PROXY_SERVICE`、`ProxyOpenReq` / `ProxyOpenResp`、PN control channel、`TunnelManager` proxy path publish 和 `PnServer` relay bridge；`TtpClient` 生命周期清理复用现有 tunnel cache pruning 与 TTP open_* 入口，不新增底层 tunnel trait。
- New abstractions introduced: `TtpNode`、`TtpIncomingTunnelValidator` / `TtpIncomingTunnelValidateContext`、`PnProxyRouteResolver` 或等价 `PnClient` 内部 route 解析配置、`PnAssignedTargetPolicy` / `PnRelayAdmission`、`PnRelaySessionRegistry`。
- Why each new abstraction is necessary: `TtpNode` 提供同时监听和按需主动建链的 TTP 节点入口，避免改变 `TtpServer` lookup-only 语义；`TtpIncomingTunnelValidator` 把 server 本地 accept/reject 策略放在 attach/cache 前并保持默认兼容；route 输入消除 latest/default PN 歧义；assigned target policy 是错误 PN 在打开目标流前失败的准入点；session registry 用于区分新建 logical tunnel 与既有 tunnel 后续双向 channel。

## Current Structure
当前 `p2p-frame` 是一个包含 `networks`、`tunnel`、`ttp`、`sn`、`pn`、`finder`、TLS/X509 和 stack 组装层的核心 crate。TTP 现有公开对象分为 `TtpClient` 和 `TtpServer`：`TtpClient` 会在缺失 target tunnel 时主动创建并 attach，`connect_server(...)` 会把 target 加入 `maintained_targets` 并由后台 maintain loop 重连，但当前没有删除保持 target 的公开入口，普通 target 创建出的 cache tunnel 也没有按 idle 生命周期释放；`TtpServer` 通过 `NetManager` incoming subscriber 记住已接收 tunnel，并在 open 路径上只查找既有 tunnel。当前 `TtpServer` incoming subscriber 对所有 `Ok(tunnel)` 默认 attach/remember，调用方无法在 server 本地拒绝某条 incoming tunnel。PN 现有路径通过 `PnClient` / `PnServer` 在 `PROXY_SERVICE` 上使用 `ProxyOpenReq` / `ProxyOpenResp` 建立 relay bridge；当前实现中存在按 latest tunnel 或单 proxy client 隐式选择 PN 的倾向。`PnServer::new(...)` 的 allow-all 便捷构造符合未配置部署兼容目标，但不能作为多 PN assigned target 准入的替代路径。
...
- `TtpServer` incoming validator 只控制当前 server 是否 attach/remember 该 tunnel，不改变底层 incoming publish；默认构造必须保持 allow-all 兼容行为。
...
| `ttp` | technical | tunnel 上的命令和流复用协议、TTP target tunnel 查找、server incoming tunnel 准入、主动建链节点、TTP client 本地连接生命周期 | `networks`, `tunnel_channel_callback`, `tunnel_control_stream` | TTP client/server/node stream APIs and server validator | 具体 tunnel attachment 由 stack/runtime 组装 |
| `sn` | business | 对端注册、信令和调用转发，SN control stream 信令，连接验证器，client 侧 QUIC-first/TCP-fallback 协议优先级，移除 contract/receipt 生产路径 | `ttp`, `identity_tls`, `tunnel_control_stream` | SN service/client APIs | 单 SN 边界 |
...
| SN client protocol priority | business | SN client decides per-SN report attempt order across available local/listener protocol pairs | ordered candidate loop inside `sn/client` over existing TTP command tunnel creation | keep in `sn/client`; do not change TTP target matching or maintained-target semantics |
...
| SN client protocol priority | SN client ping/report loop | `sn/client` -> existing TTP command tunnel factory -> `ReportSn` | 对同一 SN 按 QUIC 优先、TCP fallback 建立 command tunnel 并上报 | QUIC tunnel creation or `ReportSn` failure continues to TCP; QUIC `ReportSn` success stops TCP for that SN; duplicate active SN is not inserted | no TTP target matching or wire change |
...
| TTP server incoming tunnel validation | `NetManager` incoming subscriber callback | `ttp/server` validator -> `TtpRuntime::attach_tunnel` -> server tunnel cache | 让调用方控制当前 server 是否接受新 tunnel | validator error returns from callback without attach/cache; reject logs and does not remember the tunnel; rejected tunnel is best-effort closed; default allow-all accepts | server-local policy only; bottom-level publish and wire behavior unchanged |
...
| `sn` | P-SN-CONTRACT-CLEANUP-1 / `remove_sn_service_contract_server`; P-SN-TCP-SOURCE-MAPPED-ONLY-1 / `sn_tcp_source_mapped_only` | existing | `design.md` and SN design references | 清理与 TCP 来源地址过滤都发生在既有 SN service/client 边界内 |
...
| `ttp` | P-TTP-NODE-ACTIVE-OPEN-1 / `ttp_node_active_open`; P-TTP-SERVER-TUNNEL-ACCEPT-VALIDATOR-1 / `ttp_server_tunnel_accept_validator`; P-TTP-CLIENT-CONNECTION-LIFECYCLE-1 / `ttp_client_connection_lifecycle` | existing | `design.md` | `TtpServer` 准入、`TtpNode` 与 `TtpClient` 生命周期管理都属于既有 TTP 封装责任，不需要新的直接 submodule packet |

## Trigger Matrix
| trigger_category | applies | evidence | design_coverage | required_checks | deferred_checks_and_reason |
|------------------|---------|----------|-----------------|-----------------|----------------------------|
| contract/protocol | yes | PN multi-server constrains `ProxyOpenReq` relay behavior without changing wire; control stream adds private control command data; `TtpNode` adds a TTP wrapper API; `ttp_server_tunnel_accept_validator` adds server-local incoming tunnel policy; `ttp_client_connection_lifecycle` adds local client lifecycle without changing tunnel or TTP wire; `sn_client_protocol_priority` changes only SN client candidate ordering and fallback policy; `sn_tcp_source_mapped_only` filters SN service response candidates without changing SN wire | this file, `design/pn-server.md`, control stream docs, `ttp_node_active_open`, `ttp_server_tunnel_accept_validator`, `ttp_client_connection_lifecycle`, `sn_client_protocol_priority`, and `sn_tcp_source_mapped_only` mappings | testing must cover correct PN, wrong PN, TtpNode active open, TtpServer default accept/reject, TtpClient remove/idle lifecycle, SN client protocol fallback behavior, TCP source endpoint filtering, mapped-port construction, and no wire/trait format change | owner: testing; risk: protocol/API regression and endpoint candidate leakage; acceptance impact: verify no PN/TTP/SN wire drift |
...
| runtime/integration | yes | PN route selection, server constructor migration, control streams, TtpNode active open, TtpServer incoming validator, TtpClient maintained target removal / non-maintained idle release, SN client protocol priority, SN TCP source endpoint filtering, bounded queues and listener runtime affect integration | Directly Mapped Change Items | integration must cover specified PN success, wrong PN failure, TtpNode create/reuse behavior, TtpServer accept/reject behavior, TtpClient cache lifecycle, SN client protocol fallback behavior, SN response endpoint filtering, and migrated callers | owner: testing; risk: workspace migration and unstable SN online path selection; acceptance impact: stack callers compile and run |
...
| harness/process | yes | new change ids must map to design and admission scope, including `ttp_node_active_open`, `ttp_server_tunnel_accept_validator`, `ttp_client_connection_lifecycle`, `sn_client_protocol_priority`, and `sn_tcp_source_mapped_only` | Directly Mapped Change Items | doc-structure-check and stage-scope-check | owner: design; risk: admission mismatch; acceptance impact: change_id evidence exists |
...
| 3 | 在硬性准入检查下实现 SN 生产代码 | 已批准 design | production code and admission evidence | 阶段 2 | no |
...
| `TtpIncomingTunnelValidator` / `TtpServer::new_with_incoming_tunnel_validator(...)` | `ttp_server_tunnel_accept_validator`, TTP server callers that need local incoming tunnel admission | new | default `TtpServer::new(...)` uses allow-all; explicit validator can accept or reject before attach/cache |
...
- `TtpServer` incoming tunnel validator 是 server 本地公开装配点。新增 `TtpIncomingTunnelValidateContext`，字段来自 `TunnelRef` 元数据，至少包含 `local_id`、`remote_id`、`protocol`、`tunnel_id`、`candidate_id`、`local_ep`、`remote_ep`；不包含 TTP stream payload、purpose、PN relay payload 或下游业务字段。新增 `TtpIncomingTunnelValidator` trait 和 `TtpIncomingTunnelValidatorRef`，接口形态与现有 network/SN/PN validator 一致，返回 `ValidateResult::Accept` 或 `ValidateResult::Reject(reason)`；`allow_all_ttp_incoming_tunnel_validator()` 返回默认 allow-all。`TtpServer::new(local_identity, net_manager)` 始终使用 allow-all；显式策略通过 `TtpServer::new_with_incoming_tunnel_validator(local_identity, net_manager, validator)` 或等价 options 构造器注入。incoming callback 收到 `Ok(tunnel)` 后先构造 context 并调用 validator；`Accept` 后才调用 `runtime.attach_tunnel(tunnel.clone()).await` 和 `remember_tunnel(tunnel)`；`Reject` 或 validator error 不 attach、不 remember，并 best-effort 调用 `tunnel.close().await` 或等价 close 方法，close 失败只记录日志，不把被拒 tunnel 放入 cache。`Err(result)` 仍按现有错误日志路径处理并保持 subscriber 存活。
...
- SN server 的 TCP 来源地址过滤属于 `sn/service` 的响应候选组装逻辑。`SnService` 可以从 command tunnel metadata 临时读取 observed remote endpoint，但 `get_peer_wan_ep(...)`、`get_peer_wan_classied_ep(...)`、`ReportSnResp.end_point_array`、`SnQueryResp.end_point_array` 和 `SnCalled.reverse_endpoint_array` 不得包含协议为 TCP 的 raw observed socket address。`get_peer_wan_ep_with_map_port(...)` 或等价 helper 必须分离两类候选：非 TCP observed endpoint 继续按 `classify_observed_endpoint(...)` 返回；TCP observed endpoint 只作为 IP 来源参与 `mapped_endpoint_from_observed(observed, protocol, port)`，并且只在 `map_ports` 非空时生成 `EndpointArea::Mapped` 候选。peer cache 继续只保存客户端 report 的 desc/info/map_ports，不新增保存 raw TCP source endpoint 的字段。
- SN client 协议优先级属于 `sn/client` 的在线 ping/report 候选调度。`SNClientService::ping_proc` 或等价 helper 先按 SN 服务端聚合可尝试候选，再按协议优先级排序：QUIC 候选先于 TCP 候选，同协议内优先使用匹配 listener 派生出的本地端点候选；若没有匹配 listener，但本地 runtime/network 支持该出站协议，仍必须构造不带 `local_ep` 的出站候选尝试该 SN endpoint。对同一 SN，客户端按候选顺序调用既有 command tunnel 建链和 `ReportSn`；有匹配 QUIC listener 的候选必须把 listener `local_ep` 传入建链分类；有匹配 TCP listener 的候选必须先把本地端点规范化为不复用监听端口：如果 listener IP 是 unspecified，使用 `local_ep = None`；如果 listener IP 是具体地址，保留该 IP 但把 port 改为 `0` 让操作系统分配 ephemeral local port；未匹配 listener 的候选使用 `local_ep = None`，不得构造 fake 本地监听端点。候选建链或 report 失败必须输出包含 SN id、protocol、local_ep、remote endpoint 和错误的诊断日志，避免 TCP fallback 在本地 bind 阶段静默失败。QUIC 建链失败、control stream 打开失败或 `ReportSn` 失败时继续尝试后续候选，全部 QUIC 候选失败后才进入 TCP 候选。任一候选 `ReportSn` 成功后立即把该 SN 作为 active SN 记录，并停止该 SN 的后续协议尝试；写入 active SN 列表前必须按 `sn_peer_id` 去重，避免同一 SN 因多 endpoint/listener 或 QUIC/TCP 双成功而重复。该逻辑不得修改 `SnClientTunnelFactory::open_cmd_tunnel(...)`、`TtpClient::connect_server(...)` 的幂等 maintained-target 语义或 TTP target matching；TCP listener 端口规范化仅发生在 SN client 候选生成处，不改变 TTP target matching 或其他 TTP 调用者传入的 local endpoint 语义。
...
- 对协议为 TCP 的 SN observed endpoint，服务端不得执行上述普通 `Wan` / `ServerReflexive` 返回路径；该 endpoint 只能在客户端上报 `map_ports` 时提供来源 IP，映射候选的协议和端口必须来自 `map_ports`，area 必须为 `Mapped`，原始 TCP 来源端口不得进入返回数组。
...
| `TtpServer` incoming tunnel validator | `TtpServer` 新增 validator trait/context、allow-all helper 和显式 validator 构造路径；incoming callback 在 `TtpRuntime::attach_tunnel(...)` 与 `remember_tunnel(...)` 前执行 validator，accept 后 attach/cache，reject 或 validator error 不 attach/不 cache，并 best-effort close 被拒 tunnel | `p2p-frame/src/ttp/server.rs`、`p2p-frame/src/ttp/mod.rs`、必要 `p2p-frame/src/ttp/tests.rs` | 若实现阶段需要改变 `NetManager` / `TunnelNetwork` publish、wire 协议、TTP stream payload、`TtpNode` active-open 或 `TtpClient` lifecycle，应退回 proposal/design；若只是 validator context 字段需要补充，优先保持为 tunnel metadata。 |
...
| SN client 协议优先级 | 同一 SN 的在线 ping/report 候选按 QUIC first、TCP fallback 处理；候选尝试以本地支持出站协议为前提，不以匹配 listener 存在为前提；有匹配 QUIC listener 时必须保留 listener `local_ep`，有匹配 TCP listener 时必须把 unspecified IP 转成 `None` 或把具体 IP 的 port 转成 `0`，无匹配 listener 时用 `local_ep = None` 尝试出站连接；候选失败日志必须暴露 SN id、protocol、local_ep、remote endpoint 和错误；QUIC command tunnel 建立并 `ReportSn` 成功后停止 TCP 尝试；QUIC 建链或 report 失败后才尝试 TCP；active SN 写入按 `sn_peer_id` 去重。 | `p2p-frame/src/sn/client/sn_service.rs`、必要 `p2p-frame/src/sn/**` 测试 | 若实现阶段需要改变 SN command wire、control-stream-only 信令、TCP/QUIC tunnel wire、TTP target matching、`TtpClient::connect_server(...)` maintained target 语义、入站 listener/上报候选语义、服务端选择 API或跨 SN fanout，应退回 proposal/design；若只是候选排序 helper、本地协议能力判定 helper 或 TCP local_ep 规范化 helper 形态不清，应保持在 `sn/client` 内部。 |
...
| TTP server tunnel validation point | validate in `TtpServer` incoming callback before attach/cache | validate after attach or push policy into `NetManager` | post-attach validation leaks rejected tunnels into runtime/cache; `NetManager` policy would become a global publish rule instead of server-local acceptance |
...
| SN client protocol priority location | order candidates inside `sn/client` ping/report loop | change TTP target matching or `connect_server` maintained-target behavior | this is an SN online policy; changing TTP would affect unrelated TTP users and exceed proposal non-goals |
...
| TTP server incoming validator | `ttp` | injected into `TtpServer` explicit constructor; only server incoming callback calls it | default allow-all -> attach/cache; explicit accept -> attach/cache; explicit reject or error -> no attach/cache and best-effort tunnel close |
...
| SN client report candidates | `sn/client` | internal to `SNClientService::ping_proc` or helper; consumed only by SN online/report loop | collect per SN -> sort QUIC before TCP -> try command tunnel/report -> on success insert one active SN and stop same SN fallback -> on failure continue to next candidate |
| SN service observed endpoint candidates | `sn/service` | transient command tunnel metadata used only during response candidate construction | observed TCP endpoint -> excluded from direct return/cache; observed TCP IP plus `map_ports` -> `Mapped` candidate; observed non-TCP endpoint -> classified as `Wan` or `ServerReflexive` |
...
## Testability
- Isolation seams per submodule: TTP active open, TTP server incoming validation, and TTP client lifecycle can be tested with fake `NetManager` / tunnel network, in-memory tunnel handles, validator test doubles, and test-only short idle deadlines; SN client protocol priority can be tested through an internal ordered-candidate/report-loop helper or fake command tunnel/report outcome table; SN service TCP source filtering can be tested with fake command tunnels whose remote endpoint is TCP or QUIC plus direct helper assertions for response candidate construction; PN server admission can be tested with fake assigned target policy and fake target stream factory; PN client relay route can be tested with fake TTP connector target behavior; relay session registry can be tested without real network.
...
- How error/boundary cases will be triggered: TTP missing tunnel by starting with an empty node cache, tunnel reuse by pre-remembering an available tunnel, TtpServer reject by injecting a validator returning `ValidateResult::Reject`, validator error by a test validator returning `Err`, maintained target removal by deleting a target before a maintain tick, non-maintained idle release by advancing a test-only short idle timeout, active/pending retention by holding stream/control/datagram guards, attach/open failure by fake tunnel/network errors, SN QUIC success by a fake QUIC candidate returning report success before any TCP attempt, SN TCP fallback by a fake QUIC candidate returning create/report failure followed by TCP success, duplicate prevention by multiple successful candidates for the same `sn_peer_id`, SN TCP source filtering by pairing a TCP observed endpoint with empty and non-empty `map_ports`, wrong PN by policy reject before target open, missing route by `PnClient` configuration/parameter error, existing reverse channel by pre-registered session, no session by registry miss, full queues by small bounded capacity test config where applicable.
...
- `TtpServer` incoming validator 必须严格位于 attach/cache 前；若实现让 reject tunnel 已经进入 `TtpRuntime` 或 server cache，必须回滚该实现路径并重新设计。回滚必须恢复默认 attach/remember 所有 incoming tunnel 的兼容行为，且不得影响 `TtpNode` / `TtpClient`。
...
- SN TCP 来源地址过滤必须只影响 TCP observed endpoint 的直接返回路径；若实现误删客户端自报 endpoint、desc endpoint、非 TCP observed endpoint 或 `map_ports` 映射候选，应回滚该 helper 改动并重新拆分 direct observed 与 mapped observed 构造逻辑。回滚不得改变 SN command wire、TCP tunnel wire 或 peer manager 存储结构。
- SN client 协议优先级必须只改变同一 SN 内候选尝试顺序、listener-optional 出站候选构造、QUIC listener `local_ep` 保留、TCP listener local_ep 出站 bind 规范化、候选失败诊断日志和成功后的停止条件；若实现需要改变 TTP target matching、`connect_server(...)` 的 maintained target 语义、SN command payload、control stream purpose、入站 listener/上报候选语义或服务端选择 API，应退回 proposal/design。回滚必须恢复旧的 listener/protocol 扫描行为，但不得引入重复 active SN 记录之外的新状态语义。
...
- approval_date: 2026-07-05T10:59:25+08:00
