...
| `sn` | service | 对端注册、信令和调用转发；已有 tunnel 控制通道健康时使用 `Tunnel` control stream 承载 SN 低频小消息 | tunnel/ttp/control stream、身份 | SN 服务行为 | `ttp`、`p2p_identity`、`tunnel_control_stream` | no |
