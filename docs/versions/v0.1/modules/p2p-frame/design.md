...
approved_at: 2026-07-05T10:59:25+08:00
approved_content_sha256: 434b780b64b00d713296031069cd20a6ae2c58b2d90535f06f63b68856fa8a4b
---
...
- 为本轮 SN client 协议优先级建立可执行设计边界：同一 SN 服务端同时存在 QUIC/TCP endpoint 且本地支持对应出站连接协议时，客户端 ping/report 建链按 QUIC 优先、TCP fallback 顺序尝试；是否尝试某协议不要求本地已监听该协议端口；如果本地存在匹配协议 listener，建链分类必须继续使用该 listener `local_ep`；某个 SN 的 QUIC command tunnel 建立且 `ReportSn` 成功后停止该 SN 的 TCP 尝试；只有 QUIC 建链或 report 失败后才进入 TCP fallback，并避免同一成功 SN 重复进入 active SN 列表。
...
- SN client 协议优先级属于 `sn/client` 的在线 ping/report 候选调度。`SNClientService::ping_proc` 或等价 helper 先按 SN 服务端聚合可尝试候选，再按协议优先级排序：QUIC 候选先于 TCP 候选，同协议内优先使用匹配 listener 派生出的本地端点候选；若没有匹配 listener，但本地 runtime/network 支持该出站协议，仍必须构造不带 `local_ep` 的出站候选尝试该 SN endpoint。对同一 SN，客户端按候选顺序调用既有 command tunnel 建链和 `ReportSn`；有匹配 listener 的候选必须把 listener `local_ep` 传入建链分类，未匹配 listener 的候选使用 `local_ep = None`，不得构造 fake 本地监听端点。QUIC 建链失败、control stream 打开失败或 `ReportSn` 失败时继续尝试后续候选，全部 QUIC 候选失败后才进入 TCP 候选。任一候选 `ReportSn` 成功后立即把该 SN 作为 active SN 记录，并停止该 SN 的后续协议尝试；写入 active SN 列表前必须按 `sn_peer_id` 去重，避免同一 SN 因多 endpoint/listener 或 QUIC/TCP 双成功而重复。该逻辑不得修改 `SnClientTunnelFactory::open_cmd_tunnel(...)`、`TtpClient::connect_server(...)` 的幂等 maintained-target 语义或 TTP target matching；若实现需要改变已有 listener 的 TCP local endpoint `port=0` 兼容策略，必须先回到 design 明确 TTP target 兼容边界。
...
| SN client 协议优先级 | 同一 SN 的在线 ping/report 候选按 QUIC first、TCP fallback 处理；候选尝试以本地支持出站协议为前提，不以匹配 listener 存在为前提；有匹配 listener 时必须保留 listener `local_ep`，无匹配 listener 时用 `local_ep = None` 尝试出站连接；QUIC command tunnel 建立并 `ReportSn` 成功后停止 TCP 尝试；QUIC 建链或 report 失败后才尝试 TCP；active SN 写入按 `sn_peer_id` 去重。 | `p2p-frame/src/sn/client/sn_service.rs`、必要 `p2p-frame/src/sn/**` 测试 | 若实现阶段需要改变 SN command wire、control-stream-only 信令、TCP/QUIC tunnel wire、TTP target matching、`TtpClient::connect_server(...)` maintained target 语义、入站 listener/上报候选语义、服务端选择 API或跨 SN fanout，应退回 proposal/design；若只是候选排序 helper 或本地协议能力判定 helper 形态不清，应保持在 `sn/client` 内部。 |
...
- SN client 协议优先级必须只改变同一 SN 内候选尝试顺序、listener-optional 出站候选构造、已有 listener `local_ep` 保留和成功后的停止条件；若实现需要改变 TTP target matching、`connect_server(...)` 的 maintained target 语义、SN command payload、control stream purpose、入站 listener/上报候选语义或服务端选择 API，应退回 proposal/design。回滚必须恢复旧的 listener/protocol 扫描行为，但不得引入重复 active SN 记录之外的新状态语义。
