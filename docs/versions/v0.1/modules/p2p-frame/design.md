...
approved_at: 2026-06-30T17:12:25+08:00
approved_content_sha256: 49b2fdb980b308976ee0072c49d9316ffba423f909f54660e7d7d4be8d60154b
---
...
|------------------|---------|----------|-----------------|-----------------|----------------------------|
| contract/protocol | yes | PN multi-server constrains `ProxyOpenReq` relay behavior without changing wire; control stream adds private control command data; `TtpNode` adds a TTP wrapper API; `ttp_server_tunnel_accept_validator` adds server-local incoming tunnel policy; `ttp_client_connection_lifecycle` adds local client lifecycle without changing tunnel or TTP wire | this file, `design/pn-server.md`, control stream docs, `ttp_node_active_open`, `ttp_server_tunnel_accept_validator`, and `ttp_client_connection_lifecycle` mappings | testing must cover correct PN, wrong PN, TtpNode active open, TtpServer default accept/reject, TtpClient remove/idle lifecycle, and no wire/trait format change | owner: testing; risk: protocol/API regression; acceptance impact: verify no PN/TTP wire drift |
...
| runtime/integration | yes | PN route selection, server constructor migration, control streams, TtpNode active open, TtpServer incoming validator, TtpClient maintained target removal / non-maintained idle release, bounded queues and listener runtime affect integration | Directly Mapped Change Items | integration must cover specified PN success, wrong PN failure, TtpNode create/reuse behavior, TtpServer accept/reject behavior, TtpClient cache lifecycle, and migrated callers | owner: testing; risk: workspace migration; acceptance impact: stack callers compile and run |
...
| harness/process | yes | new change ids must map to design and admission scope, including `ttp_node_active_open`, `ttp_server_tunnel_accept_validator`, and `ttp_client_connection_lifecycle` | Directly Mapped Change Items | doc-structure-check and stage-scope-check | owner: design; risk: admission mismatch; acceptance impact: change_id evidence exists |
