...
status: approved
approved_by: user
approved_at: 2026-07-07T10:59:09+08:00
approved_content_sha256: d9b2faa02356d04f89a1ee9d856461f47cf2a42f830949d8c74e20e92fee7342
---
...
| build/dependency/config/deployment | yes | 上层指定 PN server 和 assigned target 策略会影响配置/部署边界，但不要求库内目录；`sn_service_runtime_external_injection` 要求 SN service 不再创建 service-local `ServerRuntime`，而是消费外部或 stack 级全局 runtime。 | design must define configuration/interface boundary without adding global directory; design must define required runtime API/error boundary and stack-level runtime ownership. | owner: design; risk: accidental global directory/config expansion or runtime ownership drift; acceptance impact: no library-owned PN directory and no internal SN service runtime start. |
...
- `ServerRuntime` 注入必须是显式配置能力；SN service 不得在内部自行创建 runtime。若需要保留默认启动体验，只能由 stack 或更上层装配路径创建一次全局 runtime 后向下传入，底层 service 构造不得使用 `unwrap_or_else(ServerRuntime::start(...))` 兜底。
...
- `ServerRuntime` 可由外部设置并复用于 TCP/QUIC listener；SN service 使用的 runtime 必须来自外部或 stack 级全局对象，不得在 `sn/service` 内部自行创建新的 service-local runtime。
...
- 外部设置 `ServerRuntime` 时，TCP/QUIC listener 和 SN service 必须使用该 runtime；SN service 缺少 runtime 时不得静默启动新的 `ServerRuntime`，必须按 design 定义的必填配置或错误路径处理。
...
## Approval Record
- approver: user
- approval_date: 2026-07-07T10:59:09+08:00
