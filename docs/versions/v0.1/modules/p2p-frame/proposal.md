...
status: approved
approved_by: user
approved_at: 2026-07-07T15:52:07+08:00
approved_content_sha256: 89f2871b7c9886f225f16b72203dae9f175c37a4c7a2d26e4c0465bcbe710dc0
---
...
- 本轮新增需求是收紧 SN service 的 `ServerRuntime` 所有权：`ServerRuntime` 应作为进程或 stack 级全局运行时对象由外部显式传入，`p2p-frame/src/sn/service/service.rs` 的 SN service 构造路径不得在内部自行调用 `ServerRuntime::start(...)` 创建新 runtime；缺少 runtime 必须作为配置或 API 使用错误暴露给调用方，而不是静默创建新的 service-local runtime。
- 本轮新增澄清是继续收紧 stack 级 `ServerRuntime` 所有权：`p2p-frame/src/stack.rs` 中 `P2pConfig` / stack 装配路径不得在默认构造或 `create_p2p_env(...)` 中自行调用 `ServerRuntime::start(...)` 创建 runtime；runtime 必须由外部调用方显式传入，缺少 runtime 必须作为配置或 API 使用错误暴露，而不是静默创建库内默认 runtime。
...
- `ServerRuntime` 必须由 `p2p-frame` 外部通过明确配置入口传入；未设置时 `p2p-frame` 不得创建默认 `sfo_reuseport::ServerRuntime`，必须按 design 定义返回配置/API 错误或通过必填构造参数阻止缺失 runtime
- SN service 必须消费外部传入的 `ServerRuntime` 或 stack 级已解析全局 runtime；`create_sn_service(...)`、`SnServiceConfig` 装配和 `SnServer` 构造路径不得自行启动 service-local `ServerRuntime`
...
- SN 客户端连接同一 SN 服务端时必须按协议候选优先级执行：QUIC candidate 在 TCP candidate 之前尝试；候选是否可尝试取决于本地是否支持对应出站连接协议，而不是本地是否已监听该协议端口；如果存在匹配协议的本地 listener，QUIC 候选继续使用该 listener 的 `local_ep` 和监听端口，TCP 候选必须使用 ephemeral local port 或 `local_ep = None`，不得复用已监听端口进行出站 bind；QUIC 建链或 `ReportSn` 成功后停止该 SN 的 TCP 尝试；只有 QUIC 建链或 `ReportSn` 失败时才进入 TCP fallback。
...
- 在 SN service 构造路径中通过 `ServerRuntime::start(...)`、`unwrap_or_else(...)` 或等价 fallback 静默创建 service-local runtime；缺少 runtime 必须由 design 定义为必填配置或显式错误
...
- SN service runtime 装配属于 `p2p-frame/src/sn/service/**` 与 stack 组装边界；下游启动二进制可以创建或透传全局 `ServerRuntime`，但不得依赖 SN service 内部静默创建 service-local runtime。
...
- Decision needed before approval:
  ...
  - 用户需审批 `sn_service_runtime_external_injection` 作为新的 proposal item 后，design/testing/implementation 才能继续。
...
| If a matching local listener exists, should SN client still pass its local endpoint? | QUIC 应保留 listener `local_ep`，因为 QUIC 同源端口与 NAT 打洞和反连候选语义相关；TCP 不应保留已监听端口作为出站 bind 地址，因为同一端口已由 TCP listener 占用，fallback 会在本地 bind 阶段失败。 | 如果 TCP 无条件复用 listener 端口，会导致 TCP fallback 对可达 SN TCP 地址也无法连接；如果所有协议都传 `None`，会破坏 QUIC 同源 listener 语义。 | preserve QUIC listener local_ep; use ephemeral/no local_ep for TCP fallback |
...
| Should SN service create its own `ServerRuntime` when config omits one? | 不应继续在 SN service 内部创建。用户明确指出 `ServiceRuntime` / `ServerRuntime` 是全局对象，应由外部传入；service-local fallback 会造成多个 runtime owner、生命周期不清和关闭不可控。 | 取消 service 内部 fallback 会要求调用方或 stack 组装层显式提供 runtime；design 必须定义这是构造时必填、返回配置错误，还是由更上层默认创建一次再传入。 | add `sn_service_runtime_external_injection`; no internal runtime start in SN service |
| Should stack-level `P2pConfig` create a default `ServerRuntime` when caller omits one? | 不应继续默认创建。用户明确指出 `p2p-frame/src/stack.rs:162-163` 必须由外部传入；stack 级 fallback 同样会隐藏 runtime 所有权并让 TCP/QUIC listener 生命周期不可由进程级调用方统一管理。 | 取消 stack 默认 runtime 会影响 `P2pConfig::new(...)` / `create_p2p_env(...)` 及下游调用方，design 必须定义是使用必填构造参数、builder 错误，还是新的 fallible constructor；implementation 需要迁移 workspace 直接调用方。 | add `stack_server_runtime_external_required`; no `ServerRuntime::start(...)` in `p2p-frame/src/stack.rs` production config/assembly path |
...
| sn | existing | SN report/query/call/called 候选生成、TCP 来源地址使用边界、映射端口候选构造和 SN service runtime 装配边界 | `docs/versions/v0.1/modules/p2p-frame/proposal.md` | 本需求属于既有 `p2p-frame/src/sn/**` 责任，不需要创建新的直接 submodule packet。 |

## Trigger Matrix
| trigger_category | applies | evidence | required_checks | deferred_checks_and_reason |
|------------------|---------|----------|-----------------|----------------------------|
| contract/protocol | yes | `pn_multi_server_assigned_target` 约束 PN relay 建链、assigned target 准入和错误 PN 行为，但不改变 `ProxyOpenReq` / `ProxyOpenResp` wire；`ttp_node_active_open` 新增 TTP 封装 API 行为；`ttp_server_tunnel_accept_validator` 新增 TTP server 本地 incoming tunnel 准入行为；`ttp_client_connection_lifecycle` 新增 TTP client 本地 target/tunnel 生命周期行为；`sn_client_protocol_priority` 新增 SN 客户端多协议候选选择规则，并澄清 SN 客户端出站连接候选只要求本地支持对应连接协议，不要求本地已监听该协议端口；如果本地已监听匹配协议，则 QUIC 建链分类仍必须使用 listener `local_ep`，TCP 建链分类必须改用 ephemeral/no local_ep；`sn_tcp_source_mapped_only` 收窄 SN 返回的 TCP 来源候选集合，但不得改变 SN command wire、control-stream-only 信令、TCP/QUIC/PN/TTP wire 或 `Tunnel` trait。 | design must map PN protocol invariants, TTP server/node/client lifecycle boundaries, SN client QUIC-first/TCP-fallback ordering, SN client supported-outbound-protocol gating without local-listener gating, QUIC listener `local_ep` preservation, TCP listener ephemeral/no-local endpoint normalization, and SN TCP source candidate filtering; testing must cover correct PN success/wrong PN failure, TtpServer accept/reject/default allow-all, TtpNode active open, maintained target removal, non-maintained idle release, SN client QUIC success stopping TCP fallback, SN client can attempt a supported protocol without a matching local listener, matching QUIC listener port is still passed as `local_ep`, matching TCP listener port is not reused for outbound bind, and TCP source address not directly returned. | owner: design/testing; risk: PN wire compatibility, TTP API drift, TTP lifecycle race, duplicate SN active connections, stale TCP source candidates leaking to clients, SN clients being incorrectly considered offline when they support outbound connection but do not listen locally, QUIC listener local port semantics being lost for clients that do listen, and TCP fallback failing locally by rebinding a listening port; acceptance impact: verify no wire protocol or public tunnel trait change, verify SN client outbound protocol gating is independent from local listener presence, verify QUIC listener `local_ep` is preserved when present, verify TCP listener local_ep is normalized for outbound bind, and verify only mapped candidates use TCP source IP. |
...
| build/dependency/config/deployment | yes | 上层指定 PN server 和 assigned target 策略会影响配置/部署边界，但不要求库内目录；`sn_service_runtime_external_injection` 要求 SN service 不再创建 service-local `ServerRuntime`，而是消费外部或 stack 级全局 runtime；`stack_server_runtime_external_required` 要求 `P2pConfig` / stack 装配路径也不得自行创建默认 `ServerRuntime`，而是由调用方显式传入进程级 runtime。 | design must define configuration/interface boundary without adding global directory; design must define required runtime API/error boundary and stack-level runtime ownership; testing must cover missing stack runtime failure or compile-time required parameter and explicit caller injection. | owner: design/testing; risk: accidental global directory/config expansion, runtime ownership drift, downstream constructor migration, and hidden default runtime reintroduction; acceptance impact: no library-owned PN directory, no internal SN service runtime start, and no stack-level default `ServerRuntime::start(...)`. |
...
| harness/process | yes | 新增和修订 proposal item `pn_multi_server_assigned_target`、`ttp_node_active_open`、`ttp_server_tunnel_accept_validator`、`ttp_client_connection_lifecycle`、`sn_client_protocol_priority`、`sn_tcp_source_mapped_only`、`sn_service_runtime_external_injection` 和 `stack_server_runtime_external_required` 会使下游 design/testing/implementation admission 需要重新映射。 | run doc-structure-check and stage-scope-check for proposal; downstream admission must use new change_id. | owner: downstream stages; risk: admission gap; acceptance impact: design/testing must directly map revised change_id semantics. |
...
- `ServerRuntime` 注入必须是显式配置能力；SN service 与 stack 配置/装配路径均不得在内部自行创建 runtime。若需要保留默认启动体验，只能由 `p2p-frame` 外部调用方创建一次进程级 runtime 后向下传入，`p2p-frame/src/stack.rs` 和 `sn/service` 构造不得使用 `ServerRuntime::start(...)`、`unwrap_or_else(...)` 或等价 fallback 兜底。
...
- `ServerRuntime` 必须由外部设置并复用于 TCP/QUIC listener；SN service 使用的 runtime 必须来自外部或 stack 级全局对象，不得在 `stack.rs` 或 `sn/service` 内部自行创建新的 default/service-local runtime。
...
- 外部设置 `ServerRuntime` 时，TCP/QUIC listener 和 SN service 必须使用该 runtime；stack 配置或 SN service 缺少 runtime 时不得静默启动新的 `ServerRuntime`，必须按 design 定义的必填配置、fallible constructor 或错误路径处理。
...
- 本次 proposal 已按用户要求修订 `sn_client_protocol_priority` 并由用户确认启动 auto-pipeline；本次后续修订把 TCP listener 出站 bind 冲突纳入同一 change_id。Design 必须补齐 SN client 候选收集与排序、支持的出站连接协议判定、没有匹配本地 listener 时仍可尝试 SN endpoint 的边界、QUIC listener local_ep 保留边界、TCP fallback 使用 ephemeral/no local_ep 的边界、QUIC 成功后的停止条件、QUIC 建链失败和 `ReportSn` 失败后的 TCP fallback、active SN 去重，以及该策略与 `TtpClient::connect_server(...)` 幂等登记和 TTP target 匹配之间的边界。
- Testing 必须补齐同一 SN 同时具备 QUIC/TCP endpoint 时 QUIC 优先、支持协议但没有匹配本地 listener 时仍尝试 SN endpoint、有匹配 QUIC listener 时仍使用 listener `local_ep`、有匹配 TCP listener 时 TCP fallback 不复用监听端口、QUIC 成功不尝试 TCP、QUIC 失败后 TCP fallback、同一 SN 不重复 active、以及 `connect_server` 不因成功路径被协议循环重复调用的验证映射；Implementation 只有在 proposal/design/testing 均 approved 且 admission 以 `sn_client_protocol_priority` 通过后，才能修改生产代码。
...
- 本次 proposal 已按用户要求新增 `sn_service_runtime_external_injection`，当前 proposal 回到 draft；用户审批后，Design 必须补齐 SN service runtime 必填或错误返回 API、stack 级全局 runtime 注入边界、下游启动路径迁移，以及与既有 `sfo-reuseport` listener runtime 所有权之间的关系。
- Testing 必须补齐显式传入 `ServerRuntime` 时 SN service 可启动、缺少 runtime 时按 design 定义失败、`sn/service/service.rs` 生产路径不再内部调用 `ServerRuntime::start(...)`，以及不改变 SN command wire、control-stream-only 信令、连接验证器和 TCP 来源地址过滤语义；Implementation 只有在 proposal/design/testing 均 approved 且 admission 以 `sn_service_runtime_external_injection` 通过后，才能修改生产代码。
- 本次 proposal 已按用户要求新增 `stack_server_runtime_external_required`，并由用户确认自动处理后续步骤；Design 必须补齐 `P2pConfig` / `create_p2p_env(...)` 的 runtime 必填、fallible constructor 或错误返回 API，列出 `cyfs-p2p`、`cyfs-p2p-test`、p2p-frame SN command matrix 等直接调用方迁移边界，并同步 TCP/QUIC listener 默认 runtime 表述。
- Testing 必须补齐显式传入 `ServerRuntime` 时 stack 可启动、缺少 runtime 按 design 失败或无法编译、`p2p-frame/src/stack.rs` 生产路径不再调用 `ServerRuntime::start(...)` 默认兜底、以及不改变 TCP/QUIC wire、TLS 身份校验、listener 分发和 SN service runtime 外部注入语义；Implementation 只有在 proposal/design/testing 均 approved 且 admission 以 `stack_server_runtime_external_required` 通过后，才能修改生产代码。

## Approval Record
- approver: user
- approval_date: 2026-07-07T15:52:07+08:00
