...
| target 侧统计查询不会引入 target 限速 | 待补 `pn_server` source-only 限速测试 | target 能看到自己的统计，但不会因为 target 统计视图存在而额外受到限速 |
...
- target 统计键必须与 relay 成功打开的 `req.to` 一致；测试不允许把 target 视图锚定到伪造报文或 source 身份。

## 统计与限速特定预期

- 流量统计只覆盖成功握手之后的 payload bridge，不统计 `ProxyOpenReq` / `ProxyOpenResp`。
- source 与 target 两个统计视图都必须可查询，且查询结果互不覆盖。
...
- target 侧统计能力不得引入新的 target 限速或双边配额行为。
...
| payload 统计误把控制帧或失败写入计入流量 | source 与 target 两侧统计值都只反映成功转发 payload 字节 | 尚无专门的 unit 测试覆盖 |
| target 统计错误锚定到 source 或伪造身份 | target 查询结果只归属目标用户本人，不覆盖 source 视图 | 尚无专门的 unit 测试覆盖 |
| 限速键错误地使用原始报文 `from` | 同一已认证用户的多连接共享预算，伪造报文 `from` 不会绕过限速 | 尚无专门的 unit 测试覆盖 |
| target 统计视图意外触发 target 限速 | target 查询可见，但 target 不会因为统计视图存在而出现额外背压 | 尚无专门的 unit 测试覆盖 |
