...
- DV 证据仍然是模块级 all-in-one 运行时场景。对于 `pn_server`，成功意味着 PN relay 启动和 proxy 流不会阻塞场景完成。
- Integration 证据仍然是工作区级测试套件。对于 `pn_server`，成功意味着 relay 侧 PN 改动不会破坏 `cyfs-p2p`、`cyfs-p2p-test` 或 `sn-miner-rust` 的兼容性。
