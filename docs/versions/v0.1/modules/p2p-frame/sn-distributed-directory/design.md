...
- 接入真实 `SnServer` / inter-SN transport 路径，避免只依赖 same-process registry shortcut。
...
Serving SN 与 OwnerDirectoryServer 通信必须通过 serving-facing listener 或等价的可替换 directory client/server 接口。`SnServer::new` 在配置 owner membership 时必须构造真实 `TtpInterSnClient` 并传给 `SnService` 与 `StaticOwnerDirectoryClient`，从而让 query/call/detail/relay 和 directory publish/query 优先走 SnServer/inter-SN transport；same-process `InterSnRegistry` 只能作为测试或兼容 fallback。
...
- `sn/service/service.rs` 当前 `ReportSn` 后会调用 `publish_serving_lease`，且 `SnServer::new` 将 `inter_sn_client` 置空，需要解耦 report 与 route publish 并接入 `TtpInterSnClient`。
...
| SnServer inter-SN transport vs registry fallback | technical | production cross-SN query/call/route must use real transport | `TtpInterSnClient` vs `InterSnRegistry` | wire real transport; fallback is test/compat only |
...
| Distributed detail merge | Serving SN query handler | real `TtpInterSnClient` detail query to remote servingSN | build existing `SnQueryResp` | per-serving timeout ignored; successful details are merged; registry fallback only if no TTP client exists | client API unchanged |
...
| 7 | Update sn-miner config and compatibility defaults | Phase 3-6 | owner serving endpoints, owner peer endpoints, no-config single SN behavior | Phase 6 | yes |
| 8 | Add 5x5 command matrix test support | Phase 1-7 | reusable test topology or inline test that creates 5 owner ids, 5 serving services, 5 user peers, asserts distinct serving binding, executes command matrix, and records reject/no-side-effect cases | Phase 7 | yes |
...
| SnServer transport | real `TtpInterSnClient` with registry fallback | registry-only same-process shortcut | registry-only cannot satisfy multi-process owner/serving deployment |
...
- `SnService` query/call can use fake directory/inter-SN clients to verify client-visible compatibility and real-client preference over registry fallback.
