...
|-----------|-------------|-----------------|-------------|-----------------------|
| sn_miner_startup_audit | P-1 | CLI、制品加载和服务启动拆分 | `sn-miner-rust/src/main.rs` | 维持启动职责边界，不吸收适配层逻辑 |
| sn_miner_artifact_defaults | P-2 | 配置和默认值风险面 | `sn-miner-rust/config/package.cfg`、`sn-miner-rust/src/main.rs` | 如默认值漂移需回滚具体启动路径 |
