...
|---------------|----------|---------------|----------------|
| `P-1` | 子命令族拆分与 CLI 稳定入口 | `desc-tool/src/main.rs`、`desc-tool/src/util.rs` | 保持工具边界，不吸收运行时职责 |
| `P-2` | 输出敏感和签名敏感路径 | `desc-tool/src/create.rs`、`desc-tool/src/modify.rs`、`desc-tool/src/sign.rs` | 若输出语义漂移需回滚具体子命令行为 |
