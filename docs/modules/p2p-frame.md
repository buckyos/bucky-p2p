...
- `UdpTunnelNetwork: TunnelNetwork` 专门拥有 `punch_only`、`predict_traversal_endpoints` 和 `validate_traversal_prediction`；这些 UDP traversal 能力不属于通用 `TunnelNetwork`。通用调用方通过默认返回 `None` 的 `TunnelNetwork::as_udp_tunnel_network()` 显式发现能力，当前只有 `QuicTunnelNetwork` 返回同一实例的 `Some(self)`。
...
- `src/networks/tcp/**` 负责 TCP listener、TLS accept、control/data connection 分流和 TCP tunnel registry；TCP listener 可基于 `sfo_reuseport::TcpServer` 接收入站 stream，并通过 `TunnelNetwork::listen(...)` 的回调交付新 tunnel。TCP tunnel 线协议只能通过版本化任务、显式迁移边界和双向兼容性证据变更，不得通过 PN/TTP 绕过 TLS 身份校验或 tunnel publish 语义。反向 data connection 使用 request-correlated `OpenDataConnResp` 建立“创建方已注册”屏障，屏障成功后由请求方拥有首轮 claim；取消、超时或晚到的 request-associated connection 必须退役，混合版本在业务字节前 fail closed。
...
- `src/sn/client/**` 的本地地址采集（`SnLocalIpProvider` 及其默认实现）只做“上报候选收窄”，不得扩大上报集合、不得修改内核地址表（禁止 `ip -6 addr del` 一类越权删除）：Linux 读取 `/proc/net/if_inet6`、Windows 读取 `GetAdaptersAddresses`（复用既有 `winapi` 依赖），过滤 deprecated/tentative/DAD 失败地址并对稳定地址优先排序；macOS 及其他没有平台 DAD 元数据 API 的平台保持 base 语义并保留地址，该能力差距是已知取舍。地址变更触发的即时上报、活性探测等运行时联动不属于采集面，需另行提案。
