# Task 20260614-ttp-client-tunnels-hashmap Admission Evidence

## Implementation Admission Evidence
| evidence_item | source | status | notes |
|---------------|--------|--------|-------|
| proposal_read | docs/versions/v0.1/modules/p2p-frame/proposal.md | pass | Read approved proposal item `pn_multi_server_assigned_target`, which covers caller-specified PN relay target behavior through TTP target-based connection/reuse. |
| design_read | docs/versions/v0.1/modules/p2p-frame/design.md | pass | Read approved design item `pn_multi_server_assigned_target`, whose scope paths include `p2p-frame/src/ttp/client.rs` and require use of the existing `TtpConnector::open_stream(...)` target interface. |
| change_scope_matches_request | proposal P-PN-MULTI-SERVER-ASSIGNED-TARGET-1 / design pn_multi_server_assigned_target | pass | The request changes only the internal tunnel cache used by `TtpClient` target lookup/reuse, without changing wire protocol, public API, or PN relay assignment semantics. |
| active_module_resolved | docs/versions/v0.1/modules/p2p-frame | pass | The requested file is under `p2p-frame/src/ttp/client.rs`, inside the `p2p-frame` module boundary. |
| no_chat_only_evidence | versioned docs only | pass | Implementation scope is bound to the approved proposal/design rows below; chat only supplied the concrete internal storage refinement. |
