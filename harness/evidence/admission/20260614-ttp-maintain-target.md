# Task 20260614-ttp-maintain-target Admission Evidence

## Implementation Admission Evidence
| evidence_item | source | status | notes |
|---------------|--------|--------|-------|
| proposal_read | docs/versions/v0.1/modules/p2p-frame/proposal.md | pass | Read approved proposal coverage for `pn_multi_server_assigned_target`, including explicit caller-selected relay PN routing, no automatic PN switching, and no fallback that hides wrong PN selection. |
| design_read | docs/versions/v0.1/modules/p2p-frame/design.md | pass | Read approved design coverage that binds PN client relay opens to the target interface and includes `p2p-frame/src/ttp/client.rs` in scope. |
| change_scope_matches_request | P-PN-MULTI-SERVER-ASSIGNED-TARGET-1 / pn_multi_server_assigned_target | pass | User requested that `start_maintain_loop` only check the `target` passed to `connect_server`; this is a TTP target-binding correction required by the explicit PN relay selection design. |
| active_module_resolved | docs/versions/v0.1/modules/p2p-frame | pass | Request names `p2p-frame/src/ttp/client.rs`, which belongs to the approved `p2p-frame` packet. |
| no_chat_only_evidence | versioned docs only | pass | Admission relies on the approved proposal/design mapping and scope paths, not on chat-only requirements. |
