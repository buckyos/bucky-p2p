# Task 20260615-pn-client-server-route-fallback Admission Evidence

## Implementation Admission Evidence
| evidence_item | source | status | notes |
|---------------|--------|--------|-------|
| proposal_read | docs/versions/v0.1/modules/p2p-frame/proposal.md | pass | Read approved proposal coverage for `pn_multi_server_assigned_target`, including upper-layer specified PN server selection, no library directory, no automatic PN switching, and missing target PN server failure behavior. |
| design_read | docs/versions/v0.1/modules/p2p-frame/design.md | pass | Read approved design coverage for `PnProxyRouteResolver` / equivalent target PN route input, PN client active open through the specified relay, and scope paths including `p2p-frame/src/pn/client/pn_client.rs` and `p2p-frame/src/ttp/client.rs`. |
| change_scope_matches_request | P-PN-MULTI-SERVER-ASSIGNED-TARGET-1 / pn_multi_server_assigned_target | pass | User requested `PnClient` resolver fallback to the current configured PN server address when no resolver is set, and failure when neither route source exists; this is a narrow implementation refinement of explicit PN route input and keeps missing route as an error. |
| active_module_resolved | docs/versions/v0.1/modules/p2p-frame | pass | Request points at `p2p-frame/src/pn/client/pn_client.rs`, which belongs to the `p2p-frame` PN client responsibility boundary. |
