...
|---------------|--------|--------|-------|
| proposal_read | `docs/versions/v0.1/modules/p2p-frame/proposal.md` | pass | Re-read the approved p2p-frame proposal row for `endpoint_area_server_reflexive` after the root design amendment. |
| design_read | `docs/versions/v0.1/modules/p2p-frame/design.md` | pass | Re-read the current approved p2p-frame design row for `endpoint_area_server_reflexive`; its coverage and Scope Paths remain unchanged. |
| change_scope_matches_request | proposal `P-ENDPOINT-AREA-1` / design `endpoint_area_server_reflexive` | pass | This regenerated fixture lets workspace-harness exercise the current evidence-bound admission-check interface after the approved p2p-frame design hash changed. |
| active_module_resolved | `docs/versions/v0.1/modules/p2p-frame` | pass | Active module is p2p-frame because the admitted fixture change_id belongs to the p2p-frame packet. |
| no_chat_only_evidence | approved proposal/design rows quoted below | pass | Admission relies only on current approved versioned documents and this newly bound fixture. |
