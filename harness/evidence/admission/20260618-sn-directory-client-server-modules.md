# Implementation Admission Evidence: SN Directory Client/Server Modules
...
| change_scope_matches_request | user request and approved packet | pass | User requested splitting `p2p-frame/src/sn/directory/mod.rs` client and server into two submodules; this is within `p2p-frame/src/sn/directory/**` and preserves the directory client/server boundary. |
...
| no_chat_only_evidence | proposal/design mapping tables | pass | Admission relies on approved versioned proposal/design rows, not chat-only requirements. |
