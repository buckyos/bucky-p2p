...
|---------------|--------|--------|-------|
| proposal_read | `docs/versions/v0.1/modules/p2p-frame/sn-distributed-directory/proposal.md` | pass | Approved proposal covers owner control/session state and partitioned peer route behavior currently implemented in `sn/directory/mod.rs`. |
| design_read | `docs/versions/v0.1/modules/p2p-frame/sn-distributed-directory/design.md` | pass | Approved design maps the affected owner control plane and peer route implementation to `p2p-frame/src/sn/directory/**`. |
| change_scope_matches_request | user request, `harness/custom-rules/module-interface-export-rules.md`, proposal/design scope paths | pass | User requested refactoring `p2p-frame/src/sn/directory/mod.rs` so implementation logic is not placed in `mod.rs`; this is a no-behavior-change extraction of admitted directory logic into responsibility-specific files. |
| active_module_resolved | packet front matter and file path | pass | version `v0.1`, module `p2p-frame`, submodule `sn-distributed-directory`. |
| no_chat_only_evidence | proposal/design direct rows and custom harness rule | pass | Admission relies on approved proposal/design scope for the code being moved and the checked-in custom rule for placement constraints, not chat-only evidence. |
